...
void MotionDenoiser::MotionEstimation(){
    ...
    clock_t motionEstimationBegin, motionEstimationEnd, kltBegin, kltEnd, meshflowBegin, meshflowEnd;
	for (int i = 1; i < m_frameNum; i++){
        motionEstimationBegin = clock();
		...
		meshflowEnd = clock();
		printf("meshflow : %03d ms\n", meshflowEnd-meshflowBegin);

        motionEstimationEnd = clock();
		printf("meshflow motion estimation: %03d ms\n", motionEstimationEnd-motionEstimationBegin);
	}
	printf("[DONE]\n\n");
}
...
void MotionDenoiser::Execute(){
    ...
    clock_t clockBegin, clockEnd, motionBegin, motionEnd, targetBegin, targetEnd;
    clockBegin = clock();
    MotionEstimation();
    for (int i = 0; i < m_frameNum; i++){
        ...
        AbsoluteMotion(i);
        motionEnd = clock();
        printf("absolute motion: %d ms/frame \n", motionEnd - motionBegin);

        targetBegin = clock();
        TargetFrameBuild(i, dst[i]);
        targetEnd = clock();
        printf("target frame: %d ms/frame \n\n", targetEnd - targetBegin);

        m_Counter_adder.setTo(1);
        //printf("denoised frame: %03d \n", i);
    }
    clockEnd = clock();
    printf("the total motion denoiser: %d ms/frame \n", (clockEnd - clockBegin) / m_frameNum);
}
...
void MotionDenoiser::SaveResult(char* name){
	...
	outVideoWriter.open(name, outVideoWriter.fourcc('X', 'V', 'I', 'D'), m_fps,m_size);
