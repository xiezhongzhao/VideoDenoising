...
void myKLT(const cv::Mat source, const cv::Mat target,
	...
	cornerSubPix(img0Gray, featurePtSet0, cv::Size(15, 15),
			cv::Size(-1, -1), cvTermCriteria(CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, 20, 0.03));

	vector<cv::Point2f> curfeaturePtSet;
	vector<uchar> curFlag;
	vector<float> curErrSet;
	calcOpticalFlowPyrLK(img0Gray, curImgGray,
			featurePtSet0, curfeaturePtSet, curFlag, curErrSet, cv::Size(15, 15));

	for (int p = 0; p < curErrSet.size(); p++)
		if (curErrSet[p] > 100 || curfeaturePtSet[p].x < 0
		        || curfeaturePtSet[p].y < 0 || curfeaturePtSet[p].x > img0Gray.cols
		        || curfeaturePtSet[p].y > img0Gray.rows)
			curFlag[p] = 0;
